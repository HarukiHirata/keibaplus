...
public interface ShuushiRepository extends ListCrudRepository<Shuushi, Integer> {
     ...
     * @return 収支テーブル取得結果
     */
    @Query("SELECT * FROM SHUUSHI WHERE SHUUSHI_NO = :shuushiNo AND USER_NO = :userNo AND DEL_FLG = :delFlg")
    ...
    @Query("""
            ...
            AND DEL_FLG = :delFlg
            """)
    int updateShuushi(@Param("shuushiNo") int shuushiNo,
            @Param("raceDate") LocalDate raceDate,
            @Param("courseNo") int courseNo,
            @Param("raceNo") int raceNo,
            @Param("kenshuNo") int kenshuNo,
    ...
    @Query("""
            ...
            WHERE SHUUSHI_NO = :shuushiNo
            AND USER_NO = :userNo
            AND DEL_FLG = :beforeDelFlg
            """)
    int deleteShuushi(@Param("delFlg") String delFlg,
            @Param("updDate") LocalDateTime updDate,
            @Param("shuushiNo") int shuushiNo,
            @Param("userNo") String userNo,
            @Param("beforeDelFlg") String beforeDelFlg);
